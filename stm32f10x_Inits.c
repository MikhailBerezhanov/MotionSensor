...
/*-------------- SOURCE FUNCTIONS of MotionSensor project --------------*/
...
void GPIO_init(void)
{
    ...
    GPIO_Init(GPIOC, &GPIO_InitStructure);
		GPIO_SetBits(GPIOC,GPIO_Pin_13);		//turn led OFF
    ...
    GPIO_Init(GPIOC, &GPIO_InitStructure);
		...
		/* PB0 (EXTI line 0) to recieve interrups from BMI160*/ 
...
/**
  ...
  * @retval 
  */
int8_t USART2_SendDMA(volatile char *pbuf)	
{
    strcpy(buffer, pbuf);
...
void SPI2_MASTER_init (void)
{
    ...
    PortSettings.GPIO_Speed = GPIO_Speed_50MHz;		//MOSI
    PortSettings.GPIO_Pin = SPI_MOSI_PIN | SPI_SCK_PIN;
    PortSettings.GPIO_Mode = GPIO_Mode_AF_PP;
    GPIO_Init(SPI_GPIO_PORT,&PortSettings);

		PortSettings.GPIO_Pin = SPI_MISO_PIN;				//Choose 14 pin for MISO
		PortSettings.GPIO_Mode = GPIO_Mode_IN_FLOATING;			//Input floating
		PortSettings.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(SPI_GPIO_PORT, &PortSettings);	
	
    PortSettings.GPIO_Speed = GPIO_Speed_50MHz;		//NSS
    PortSettings.GPIO_Pin =  SPI_CSB_PIN;
  ...
  SPI_InitTypeDef SPI;
    ...
    SPI.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4; //(APB1 = 36 MHz) 
