...
int8_t user_aux_read(uint8_t id, uint8_t reg_addr, uint8_t *aux_data, uint16_t len)
{
    ...
    rsltBMI160  = bmi160_aux_read(reg_addr, aux_data, len, &bmi160);
...
int8_t user_aux_write(uint8_t id, uint8_t reg_addr, uint8_t *aux_data, uint16_t len)
{
	...
	/* Discarding the parameter id as it is redundant */
	rsltBMI160  = bmi160_aux_write(reg_addr, aux_data, len, &bmi160);
...
int8_t BMI160_display_status (void)
{
	...
	USART1_SEND("\t> GYRO SELF TEST:\tF A I L E D.\r\n");
	else USART1_SEND("\t> GYRO SELF TEST:\tSUCCESSFULL\r\n");
	...
	USART1_SEND("\r\n\t> BMI160 2nd Intrf init\tSUCCESSFULL");
	else USART1_SEND("\r\n\t> BMI160 2nd Intrf init\tF A I L E D\t\r\n");
	
	/* BMM150 initialization results */
	if (rsltBMM150init == BMM150_OK)
	USART1_SEND("\r\n\t> BMM150 init\t\tSUCCESSFULL\r\n");
	else USART1_SEND("\r\n\t> BMM150 init\t\tF A I L E D\r\n");
...
{			
      ...
      USART1_SEND(USART1_TxBuf);			
			...
			if (rsltBMI160  == BMI160_OK) return (BMI160_OK);
			return 0;
}
...
{ 
	...
	SPI2->DR;		//??????? ??????? ????? ????, ?? ??? ???? ?? ????????
	//drop CSB pin = start conversation
	GPIOA->BRR = GPIO_Pin_1;									//GPIO_ResetBits(SPI_GPIO_PORT,SPI_CSB_PIN);	
  ...
  while ((SPI2->SR & SPI_I2S_FLAG_RXNE) == (uint16_t)RESET);	//while (SPI_I2S_GetFlagStatus(SPI2,SPI_I2S_FLAG_RXNE) == RESET){}	
	...
	*data = (uint8_t)SPI2->DR;									//*data = (uint8_t)SPI_I2S_ReceiveData(SPI2);
	...
	//Stop conversation
	GPIOA->BSRR = GPIO_Pin_1;									//GPIO_SetBits(SPI_GPIO_PORT,SPI_CSB_PIN);
	return BMI160_OK; 
}
...
int8_t SPI2_Write_BMM150 (uint8_t id, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
	...
	return BMI160_OK;
        ...
        rsltBMM150  = bmm150_set_op_mode(&bmm150);

		bmm150.settings.preset_mode= BMM150_PRESETMODE_LOWPOWER;
		rsltBMM150  = bmm150_set_presetmode(&bmm150);	
		*/
...
/*
	...
	rsltBMI160  = bmi160_config_aux_mode(&bmi160);
	
	// Set the auxiliary sensor to auto mode 
	rsltBMI160  = bmi160_set_aux_auto_mode(&aux_addr, &bmi160);

	// Reading data from BMI160 data registers 
	rsltBMI160  = bmi160_read_aux_data_auto_mode(mag_data, &bmi160);

	// Compensating the raw mag data available from the BMM150 API 
	rsltBMI160  = bmm150_aux_mag_data(mag_data, &bmm150);
